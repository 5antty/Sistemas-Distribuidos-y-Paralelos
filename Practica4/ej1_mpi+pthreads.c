...
void *multHybrid(void *arg)
{
    ...
    // Filas que le corresponden a este proceso MPI
    int nFilasProceso = (N * N / nProcs) / N;
    // Filas que le corresponden a este thread dentro del proceso
    int filasPorThread = nFilasProceso / nThreads;

    int ini = (*id) * filasPorThread;
    int fin = ini + filasPorThread;

    printf("Proceso %d Thread %d: filas [%d, %d)\n", idProc, *id, ini, fin);
...
void createThreads()
{
    ...
    {
        ids[i] = i;
        pthread_create(&threads[i], NULL, multHybrid, (void *)&ids[i]);
    }

    // printf("proceso %d esperando a que terminen los threads...\n", idProc);
    //  Espera a que los threads terminen

    // POR ALGUNA RAZON A PARTIR DE ACA HAY ERROR
    for (int i = 0; i < nThreads; i++)
    {
        pthread_join(threads[i], NULL);
    }

    // printf("Proceso %d todos los threads terminaron.\n", idProc);
}